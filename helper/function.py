...
from userbot import Config, arankub
from userbot.core.managers import edit_or_reply
from userbot.helpers import _arankutils

from .vcp_helper import arankVC
...
async def handler(_, update):
    ...
    if vc_player.BOTMODE:
        if resp and type(resp) is list:
            ...
            event = await arankub.tgbot.send_file(
                vc_player.CHAT_ID, file=resp[0], caption=caption, buttons=buttons
            )
        elif resp and type(resp) is str:
            resp = resp.split(f"\n\n")[1] if f"\n\n" in resp else resp
            event = await arankub.tgbot.send_message(
                vc_player.CHAT_ID, resp, buttons=buttons
            )
    else:
        try:
            results = await arankub.inline_query(Config.TG_BOT_USERNAME, "vcplayer")
            event = await results[0].click(vc_player.CHAT_ID, hide_via=True)
        except Exception:
            if resp and type(resp) is list:
                caption = resp[1].split(f"\n\n")[1] if f"\n\n" in resp[1] else resp[1]
                event = await arankub.send_file(
                    vc_player.CHAT_ID, file=resp[0], caption=caption, buttons=buttons
                )
            elif resp and type(resp) is str:
                resp = resp.split(f"\n\n")[1] if f"\n\n" in resp else resp
                event = await arankub.send_message(
                    vc_player.CHAT_ID, resp, buttons=buttons
                )
...
async def check_vcassis(event):
    chat = await arankub.get_entity(event.chat_id)
    participants = await arankub.get_participants(chat)
    assis = await vc_player.client.get_me()
    arank_ub = await arankub.get_me()
    get_id = assis.id
...
async def vc_reply(event, text, file=False, firstmsg=False, dlt=False, **kwargs):
    me = await arankub.get_me()
    if vc_player.BOTMODE:
        try:
            if file:
                arankevent = await arankub.tgbot.send_file(
                    event.chat_id, file=file, caption=text, **kwargs
                )
            else:
                arankevent = (
                    await arankub.tgbot.send_message(event.chat_id, text, **kwargs)
                    if firstmsg
        ...
        try:
            ...
            arankevent = await results[0].click(event.chat_id, hide_via=True)
        except Exception:
            arankevent = await arankub.send_file(
                event.chat_id, file=file, caption=text, **kwargs
            )
    elif vc_player.PUBLICMODE:
        arankevent = (
            await arankub.send_message(event.chat_id, text, **kwargs)
            if firstmsg and event.sender_id != me.id
            else await event.edit(text, **kwargs)
        )
    else:
        arankevent = await edit_or_reply(event, text)
    if dlt:
        await asyncio.sleep(dlt)
        return await arankevent.delete()
    if vc_player.CLEANMODE and not firstmsg:
        vc_player.EVENTS.append(arankevent)
    else:
        return arankevent
